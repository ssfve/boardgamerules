...
var stack_seg_header = '<button class="mui-navigate-right" href="#">%data%</button>';
var stack_seg_content = '<div class="mui-collapse-content">%data%</div>';

var link_seg = '<a id="%link%" class="mui-navigate-right">%data%</a>';
...
var generateside = function(arrayEN,arrayCN){
	arrayEN.forEach(function(val,index){
		name_data = arrayCN[index] +' '+ arrayEN[index];
...
var gotoPage = function(nameEN){
	...
	document.getElementById(nameEN).addEventListener('tap', function() {
		nameEN = document.getElementById(nameEN).id
		window.location.href = address_seg.replace('%data%',nameEN);
	});
